...
void tbl_read_csc (
  ...
  aux_csc_values = (float*) _mm_malloc (current_values_size * sizeof(float) , MEM_LINE_SIZE );

  // JA  points to column starts in A
  int* aux_csc_ja;
  aux_csc_ja = (int*) _mm_malloc (current_values_size * sizeof(int) , MEM_LINE_SIZE );

  // IA splits the array A into rows
  int* aux_csc_ia;
  aux_csc_ia = (int*) _mm_malloc ((current_values_size+1) * sizeof(int) , MEM_LINE_SIZE );
