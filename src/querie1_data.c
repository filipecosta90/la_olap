...
int main( int argc, char* argv[]){
  ...
  //read return flag
  tbl_read( "__tbl_2/lineitem.tbl" , 9, &returnFlag_nnz, &returnFlag_rows, &returnFlag_columns, &returnFlag_csr_values, &returnFlag_JA, &returnFlag_IA);
  convert_and_write_to_mx("__quark_mx_2/return_flag_2.mx", returnFlag_csr_values, returnFlag_JA, returnFlag_IA, returnFlag_nnz, returnFlag_rows, returnFlag_columns);

  //read line status
  tbl_read( "__tbl_2/lineitem.tbl" , 10, &lineStatus_nnz, &lineStatus_rows, &lineStatus_columns , &lineStatus_csr_values, &lineStatus_JA, &lineStatus_IA);
  convert_and_write_to_mx("__quark_mx_2/line_status_2.mx", lineStatus_csr_values, lineStatus_JA, lineStatus_IA, lineStatus_nnz, lineStatus_rows, lineStatus_columns);

  //read quantity
  tbl_read_measure( "__tbl_2/lineitem.tbl" , 5, &quantity_nnz, &quantity_rows, &quantity_columns , &quantity_csr_values, &quantity_JA, &quantity_IA);
  convert_and_write_to_mx("__quark_mx_2/quantity_2.mx", quantity_csr_values, quantity_JA, quantity_IA, quantity_nnz, quantity_rows, quantity_columns);


  //read shipdate gt
  tbl_read_filter( "__tbl_2/lineitem.tbl" , 11, GREATER_EQ , "1998-08-28",
  &shipdate_gt_nnz, &shipdate_gt_rows, &shipdate_gt_columns , &shipdate_gt_csr_values, &shipdate_gt_JA, &shipdate_gt_IA);
  convert_and_write_to_mx("__quark_mx_2/shipdate_gt_2.mx", shipdate_gt_csr_values, shipdate_gt_JA, shipdate_gt_IA, shipdate_gt_nnz, shipdate_gt_rows, shipdate_gt_columns);

  //read shipdate lt
  tbl_read_filter( "__tbl_2/lineitem.tbl" , 11, LESS_EQ , "1998-12-01",
  &shipdate_lt_nnz, &shipdate_lt_rows, &shipdate_lt_columns , &shipdate_lt_csr_values, &shipdate_lt_JA, &shipdate_lt_IA);
  convert_and_write_to_mx("__quark_mx_2/shipdate_lt_2.mx", shipdate_lt_csr_values, shipdate_lt_JA, shipdate_lt_IA, shipdate_lt_nnz, shipdate_lt_rows, shipdate_lt_columns);
